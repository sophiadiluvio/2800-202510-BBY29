    ...
    return (
        <main className="min-h-screen bg-white text-black font-sans flex flex-col">
            ...
            <div className="flex flex-col items-center justify-center flex-grow mt-10 space-y-4 text-center">
                <h2 className="text-lg font-semibold text-blue-600">Initialize Inventory for {shelter?.name || '...'}</h2>
                ...
                >
                    ...
                    <div className="flex gap-2 items-center">
                        <select value={selectedCategory} onChange={(e) => setSelectedCategory(e.target.value)} className="bg-gray-200 px-4 py-2 rounded">
                            <option value="">Select Category</option>
                            {SHELTER_CATEGORIES.filter(c => !(c in inventoryList)).map(category => (
                                <option key={category} value={category}>{category}</option>
                            ))}
                        </select>
                        <button type="button" onClick={handleAddCategory} className="bg-green-500 text-white px-4 py-1 rounded">
                            Add
                        </button>
                    </div>
