...
export default function DonationPage() {
  ...
  // load the shelter object
  useEffect(() => {
    if (shelterName) {
      fetch("/api/shelter")
        .then(res => res.json())
        .then((data: any[]) => {
          const found = data.find(s => s.name === shelterName);
          if (found) setSelectedShelter(found);
        });
    }
  }, [shelterName]);

  // **filter out zero‐requests here**
  useEffect(() => {
    if (!selectedShelter) return;

    // 1. grab the raw req object (might be undefined)
    const rawReq: Record<string, number> = selectedShelter.req || {};

    // 2. filter to only entries where amount > 0
    const filteredReq = Object.fromEntries(
      Object.entries(rawReq).filter(([_, amt]) => amt > 0)
    );

    // 3. set requestedItems to that filtered list
    setRequestedItems(filteredReq);

    // 4. initialize donation state only for those same keys
    const initialDonation = Object.keys(filteredReq).reduce((acc, key) => {
      acc[key] = 0;
      return acc;
